...
const add = async (author: string, title: string) => {
	try {
		const docRef = await addDoc(collection(db, constants.BOOK_COLLECTION), { author, title });
...
// TODO : better error handling
const getAll = async () => {
	const querySnapshot = await getDocs(collection(db, constants.BOOK_COLLECTION));
